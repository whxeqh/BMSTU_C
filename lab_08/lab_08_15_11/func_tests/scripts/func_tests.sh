...
SRC_OUT="$SCRIPT_DIR"/../../out/out.txt
...
run_positive_tests() {
    ...
    for args_file in "$SCRIPT_DIR"/../data/pos_*_args.txt; do
        ...
        if "$SCRIPT_DIR"/pos_case.sh "$args_file" "$use_valgrind"; then
...
run_negative_tests() {
    ...
    for args_file in "$SCRIPT_DIR"/../data/neg_*_args.txt; do
        ...
        in_file="${args_file/_args/_in}"
        out_file="${args_file/_args/_out}"
        if "$SCRIPT_DIR"/neg_case.sh "$args_file" "$use_valgrind"; then
