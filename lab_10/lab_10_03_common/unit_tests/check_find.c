...
START_TEST(empty_array)
{
    ...
    data_t elem_to_find = data_arr[0];
    int *value;
...
START_TEST(arr_one_elem)
{
    ...
    ck_assert_int_eq(rc, ASSOC_ARRAY_OK);
    ck_assert_int_eq(*value, elem_to_find.value);
...
START_TEST(arr_two_elems)
{
    ...
    data_t elem_to_find = data_arr[1];
    int *value;
...
START_TEST(arr_six_elems)
{
    ...
    data_t data_arr[] = { { "a", 10 }, { "c", 20 }, { "d", 30 }, { "b", 40 }, { "t", 50 }, { "e", 50 } };
    data_t elem_to_find = data_arr[1];
    int *value;
...
START_TEST(find_in_mid)
{
    ...
    data_t data_arr[] = { { "b", 10 }, { "c", 20 }, { "d", 30 }, { "e", 40 }, { "f", 50 } };
    data_t elem_to_find = data_arr[2];
    int *value;
...
START_TEST(invalid_param_key)
{
    assoc_array_error_t rc;

    assoc_array_t ass_arr;
    ass_arr = assoc_array_create();
    ck_assert_ptr_nonnull(ass_arr);
    int *value;

    rc = assoc_array_find(ass_arr, NULL, &value);
...
START_TEST(empty_key)
{
    ...
    int *value;
    rc = assoc_array_find(ass_arr, elem.key, &value);
    ck_assert_int_eq(rc, ASSOC_ARRAY_INVALID_PARAM);
...
START_TEST(num_null)
{
    ...
    rc = assoc_array_find(ass_arr, elem.key, NULL);
    ck_assert_int_eq(rc, ASSOC_ARRAY_INVALID_PARAM);
...
Suite *test_assoc_array_find(void)
{
    Suite *suite;
    TCase *tc_pos, *tc_neg;

    suite = suite_create("assoc_array_find");
    ...
    tcase_add_test(tc_neg, invalid_param_key);
    tcase_add_test(tc_neg, not_found);
    tcase_add_test(tc_neg, empty_key);
    tcase_add_test(tc_neg, empty_array);
