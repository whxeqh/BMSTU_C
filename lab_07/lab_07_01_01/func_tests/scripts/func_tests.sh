...
run_positive_tests() {
    ...
    for in_file in "$SCRIPT_DIR"/../data/pos_*_in.txt; do
        ...
        if [ -f "$args_file" ]; then
            if "$SCRIPT_DIR"/pos_case.sh "$in_file" "$out_file" "$args_file" "$use_valgrind"; then
                ...
                if $verbose; then
                    ...
                    echo -e "Actual output:\n$(sed 's/^/\t/' out/output.txt)"
                    echo
                fi
            fi
        else
            if "$SCRIPT_DIR"/pos_case.sh "$in_file" "$out_file" "$use_valgrind"; then
                ((positive_tests_passed++))
                if $verbose; then
                    echo -e "$(basename "$in_file"): \e[32mPASSED\e[0m"
                fi
...
run_negative_tests() {
    ...
    for in_file in "$SCRIPT_DIR"/../data/neg_*_in.txt; do
        ...
        args_file="${in_file/_in/_args}"
        if [ -f "$args_file" ]; then
            if "$SCRIPT_DIR"/neg_case.sh "$in_file" "$args_file" "$use_valgrind"; then
            ...
            if "$SCRIPT_DIR"/neg_case.sh "$in_file" "$use_valgrind"; then
                ((negative_tests_passed++))
                if $verbose; then
                    echo -e "$(basename "$in_file"): \e[32mPASSED\e[0m"
                fi
                ...
                if $verbose; then
                    echo -е "$(basename "$in_file"): \e[31mFAILED\e[0m"
                    echo
                    echo -е "Input data:\n$(sed 's/^/\t/' "$in_file")"
                    echo -е "Actual output:\n$(sed 's/^/\t/' output.txt)"
                    echo
